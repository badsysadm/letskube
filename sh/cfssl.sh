...
EOF
  cat > ${PKI_CA_FOLDER}/config/csr.json <<EOF
{
  "CN": "Kubernetes",
  "key": {
    "algo": "rsa",
...
EOF
  ${PREFIX}/bin/cfssl gencert -initca ${PKI_CA_FOLDER}/config/csr.json | ${PREFIX}/bin/cfssljson -bare ${PKI_CA_FOLDER}/ca
fi
...
function gen_csr_config() {
  ...
  PKI_CN=${2}
  PKI_O=${3}
  cat > ${PKI_KEY_CONFIG_FOLDER}/${PKI_NAME}-csr.json <<EOF
{
  "CN": "${PKI_CN}",
  "key": {
    "algo": "rsa",
